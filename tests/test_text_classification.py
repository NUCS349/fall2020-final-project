...
def test_dbpedia():
    ...
    model.train([x['text'] for x in train_exs], [int(x['label'])-1 for x in train_exs])

    preds = model.predict([x['text'] for x in test_exs])
    accuracy = sum((1 if pred == int(x['label'])-1 else 0) for pred, x in zip(preds, test_exs)) / len(test_exs)
...
def test_bag():
    ...
    train_exs, dev_exs, test_exs = load_data('data/bag')
